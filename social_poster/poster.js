...
async function run() {
    ...
    // 1. POST TO X (TWITTER) - SKIPPED FOR NOW
    /*
    try {
    ...
    */
    ...
    try {
        ...
        let clickedFbPost = await fbPage.evaluate(() => {
            const btns = Array.from(document.querySelectorAll('div[aria-label="Post"], div[aria-label="نشر"]'));
            // Find the active Post button in the main composer
            const postBtn = btns.find(b => b.getBoundingClientRect().width > 0 && !b.getAttribute('aria-disabled'));
            if (postBtn) {
                postBtn.click();
                return true;
            }
        ...
        if (!clickedFbPost) throw new Error('Could not find FB Post button');
    ...
    try {
        ...
        let igFileInputs = await igPage.$$('input[type="file"]');
        if (igFileInputs.length > 0) {
            await igFileInputs[igFileInputs.length - 1].uploadFile(imagePath);
        ...
        try {
            await igPage.type('div[aria-label="Write a caption..."], div[aria-label="اكتب تعليقاً..."]', caption, { delay: 10 });
    ...
    try {
        ...
        if (tkImagePath.includes('1350')) {
            tkImagePath = tkImagePath.replace('1350', '1920');
            console.log('Switched to vertical 1920 image for TikTok:', tkImagePath);
        }
        ...
        execSync(`ffmpeg -loop 1 -i "${tkImagePath}" -c:v libx264 -t 5 -pix_fmt yuv420p -vf scale=1080:1920 "${tkVideoPath}" -y`);
